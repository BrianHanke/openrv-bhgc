...
void
RvTopViewToolBar::makeActive(bool b)
{
    if (b && !m_viewBackAction) build();
    setVisible(b);
}

void
RvTopViewToolBar::makeActiveFromSettings()
{
    RV_QSETTINGS;
...
void
RvTopViewToolBar::build()
{
    if (m_viewBackAction) return;
...
void 
RvTopViewToolBar::setSession(Session* s)
{
    m_session = s;
...
void
RvTopViewToolBar::updateActionToolButton(QAction* action,
                                         const string& text,
                                         const string& icon,
                                         bool forceRepaint)
{
...
EventNode::Result 
RvTopViewToolBar::receiveEvent(const Event& event)
{
                        ...
                        if (module->name() == moduleName)
                        {
                            ...
                            for (size_t q = 0; q < devices.size(); q++)
                            {
                                ...
                                if (device->name() == deviceName)
                                {
                                    ...
                                    updateActionToolButton(m_monitorMenuAction,
                                                           deviceName,
                                                           "view_display.png");
                                    return EventAcceptAndContinue;
                                }
...
void 
RvTopViewToolBar::nextViewNode()
{
    string name = m_session->nextViewNode();

    if (!name.empty()) m_session->setViewNode(name);
}

void 
RvTopViewToolBar::previousViewNode()
{
...
bool
RvTopViewToolBar::isInPresentationMode() const
{
...
void
RvTopViewToolBar::frame()
{
    m_session->userGenericEvent("remote-eval", "extra_commands.frameImage()");
}

void
RvTopViewToolBar::fullscreen()
{
    m_session->userGenericEvent("remote-eval", "extra_commands.toggleFullScreen()");
}

void
RvTopViewToolBar::viewMenuChanged(QAction *action)
{
    if (!m_session) return;
...
void
RvTopViewToolBar::frameMenuChanged(QAction *action)
{
    if (!m_session) return;
...
void
RvTopViewToolBar::viewMenuUpdate()
{
    if (!m_session) return;
...
void 
RvTopViewToolBar::bgMenuUpdate2()
{
    ...
    {
      default:
      case ImageRenderer::Solid0:     iconName = ":/images/black.png";      break;
      case ImageRenderer::Solid18:    iconName = ":/images/grey18.png";     break;
      case ImageRenderer::Solid50:    iconName = ":/images/grey50.png";     break;
      case ImageRenderer::Solid100:   iconName = ":/images/white.png";      break;
      case ImageRenderer::Checker:    iconName = ":/images/checker.png";    break;
      case ImageRenderer::CrossHatch: iconName = ":/images/cross_hatch.png"; break;
    }
...
// void
// RvTopViewToolBar::stereoMenuUpdate()
// {
...
//     else
//     {
//         // If not in stereo/presentation mode gray out/disable the Global Swap
//         // entry.
...
// void
// RvTopViewToolBar::setStereo(const string& type)
// {
...
//     if (kickCache) m_session->setCaching(Session::NeverCache);
...
//     if (kickCache) m_session->setCaching(m);
// }

void
RvTopViewToolBar::setChannel(int c)
{
...
void
RvTopViewToolBar::channelMenuUpdate()
{
    ...
    if (hasStandardDisplayPipeline())
    {
        ...
        {
          default:
          case 0: a = m_RGBChannelAction; break;
          case 1: a = m_RChannelAction; break;
          case 2: a = m_GChannelAction; break;
          case 3: a = m_BChannelAction; break;
          case 4: a = m_AChannelAction; break;
          case 5: a = m_LChannelAction; break;
        }
...
bool 
RvTopViewToolBar::hasStandardDisplayPipeline()
{
    if (!m_session) return false;

    StringPropertyEditor displayPipeline(m_session->graph(),
                                         m_session->currentFrame(),
    ...
    {
        if (pipelineNodes[i] == "RVDisplayColor") return true;
    }
...
void
RvTopViewToolBar::monitorMenuUpdate()
{
    ...
    for (size_t i = 0; i < modules.size(); i++)
    {
        ...
        for (size_t q = 0; q < devices.size(); q++)
        {
            ...
            {
                icon = "<img src=\":/images/view_display_flat.png\" width=24 height=24>";
            }
            else
            {
                icon = " ";
            ...
            if (format.width > 0 && format.height > 0)
            {
                mon = QString("<tr>"
                              ...
                              "<td><strong>%1</strong><br><small>%2 x %3 @ %4Hz<br>%5</small>")
                    .arg(name)
                    .arg(format.width)
                    .arg(format.height)
                    .arg(format.hz)
...
void
RvTopViewToolBar::monitorMenuOCIOUpdate()
{
}

void
RvTopViewToolBar::monitorMenuRVUpdate()
{
    IntPropertyEditor srgb(m_session->graph(),
                           m_session->currentFrame(),
                           "@RVDisplayColor.color.sRGB");

    IntPropertyEditor rec709(m_session->graph(),
                             m_session->currentFrame(),
                             "@RVDisplayColor.color.Rec709");

    FloatPropertyEditor gamma(m_session->graph(),
                              m_session->currentFrame(),
                              "@RVDisplayColor.color.gamma");

    Vec2fPropertyEditor white(m_session->graph(),
                              m_session->currentFrame(),
                              "@RVDisplayColor.chromaticities.white");

    Vec2fPropertyEditor red(m_session->graph(),
                            m_session->currentFrame(),
                            "@RVDisplayColor.chromaticities.red");

    Vec2fPropertyEditor green(m_session->graph(),
                              m_session->currentFrame(),
                              "@RVDisplayColor.chromaticities.green");

    Vec2fPropertyEditor blue(m_session->graph(),
                             m_session->currentFrame(),
    ...
    IntPropertyEditor active(m_session->graph(),
                             m_session->currentFrame(),
                             "@RVDisplayColor.chromaticities.active");

    IntPropertyEditor adoptedNeutral(m_session->graph(),
                                     m_session->currentFrame(),
                                     "@RVDisplayColor.chromaticities.adoptedNeutral");

    IntPropertyEditor dither(m_session->graph(),
                             m_session->currentFrame(),
    ...
    bool none = true;
    if (srgb.value() == 1) { m_srgbAction->setChecked(true); none = false; };
    if (rec709.value() == 1) { m_rec709Action->setChecked(true); none = false; }
    ...
    Chromaticities<float> currentChr(red.value(),
                                     green.value(),
                                     blue.value(),
                                     white.value());
...
void
RvTopViewToolBar::setTransferFunction(const string& name, float value)
{
    IntPropertyEditor srgb(m_session->graph(),
                           m_session->currentFrame(),
                           "@RVDisplayColor.color.sRGB");
...
void
RvTopViewToolBar::setPrimaries(const Chromaticities<float>& chr)
{
    Vec2fPropertyEditor white(m_session->graph(),
                              m_session->currentFrame(),
                              "@RVDisplayColor.chromaticities.white");
...
void
RvTopViewToolBar::transferNone()
{
    setTransferFunction("none", 0);
}

void
RvTopViewToolBar::transferSRGB()
{
    setTransferFunction("srgb", 0);
}

void
RvTopViewToolBar::transferRec709()
{
    setTransferFunction("rec709", 0);
}

void
RvTopViewToolBar::transfer22()
{
    setTransferFunction("gamma", 2.2);
}

void
RvTopViewToolBar::transfer24()
{
    setTransferFunction("gamma", 2.4);
}

void
RvTopViewToolBar::transfer26()
{
    setTransferFunction("gamma", 2.6);
}

void
RvTopViewToolBar::primaries709()
{
    setPrimaries(Chromaticities<float>::Rec709());
}

void
RvTopViewToolBar::primariesP3()
{
    setPrimaries(Chromaticities<float>::P3());
}

void
RvTopViewToolBar::primariesSMPTEC()
{
    setPrimaries(Chromaticities<float>::SMPTE_C());
}

void
RvTopViewToolBar::primariesXYZ()
{
    setPrimaries(Chromaticities<float>::XYZ());
}

void
RvTopViewToolBar::primariesAdobeRGB()
{
    setPrimaries(Chromaticities<float>::AdobeRGB());
}

void
RvTopViewToolBar::primariesDreamColor()
{
    setPrimaries(Chromaticities<float>::DreamcolorFull());
}

void
RvTopViewToolBar::ditherOff()
{
    IntPropertyEditor editor(m_session->graph(),
                             m_session->currentFrame(),
                             "@RVDisplayColor.color.dither");
...
bool 
RvTopViewToolBar::event(QEvent* qevent)
{
...
void 
RvTopViewToolBar::setFullscreen(bool fullscreen)
{
