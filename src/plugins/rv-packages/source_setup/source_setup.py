...
class SourceSetupMode(rvtypes.MinorMode):
    ...
    def sourceSetup(self, event, noColorChanges=False):
        ...
        if ext == "DPX":
            ...
            if igPrim:
                srcInfo["TransferFunction"] = "sRGB"
        # Pre-OCIO 2 LUT workaround - no longer needed
        # elif ext == "EXR":
        #     data=["rough","disp","spec","metal"]
        #     if any(x in fileName for x in data):
        #         srcInfo["TransferFunction"] = ""
        #     else:
        #         if "color" in fileName:
        #             srcInfo["TransferFunction"] = "Linear"
        #         else:
        #             lutfile="C:/onedrive/main_projects/data/ocio/acescg-srgb.csp"
        #             commands.readLUT(lutfile, "#RVDisplayColor", True)
