...
def ocio_node_from_media(config, node, default, media=None, attributes={}):
    ...
    if nodeType == "RVDisplayPipelineGroup":
        ...
        sgb_neutral = [".jpg", ".jpeg", ".png"]
        sgb_linear = [".tiff", ".tif", ".hdr"]
        tokens = ["sss", "disp", "opac", "trans", "rough"]

        if any(x in media for x in sgb_neutral):
            parsed_colorspace = "sRGB neutral"
        elif any(x in media for x in sgb_linear):
            parsed_colorspace = "sRGB linear"
        elif ".exr" in media:
            parsed_colorspace = "ACEScg"

        elif "color" in media:
            parsed_colorspace = "ACEScg"
        elif any(x in media for x in tokens):
            parsed_colorspace = "Raw"
        ...
        result = [
            {
                ...
                "properties": {
                    "ocio.function": "color",
                    "ocio.inColorSpace": parsed_colorspace,
